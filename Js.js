...
// متغيرات إيقاف الاستماع اللحظي
let unsubscribeProducts = null;
let unsubscribeMovements = null;
let unsubscribeReturns = null;
let unsubscribeArchive = null;
let unsubscribeFactoryOrders = null;

function detachAllCompanyListeners() {
    ...
    if (unsubscribeFactoryOrders) unsubscribeFactoryOrders();
}
...
/**
 * 💡 [دالة ذكية لعزل المستخدمين حسب كود الشركة]
 * تقوم بدمج كود الشركة مع اسم المستخدم لإنشاء حساب فريد في Firebase
 */
window.encodeUserForEmail = (companyId, username) => {
    let combinedStr = companyId.toLowerCase() + "_" + username;
    let hex = "";
    
    for (let i = 0; i < combinedStr.length; i++) {
        hex += combinedStr.charCodeAt(i).toString(16);
    }
    
    return "user_" + hex + "@mastech.com";
};

/**
 * 💡 [دالة تشغيل الاستماع اللحظي للشركة الحالية فقط]
 */
function initCompanyDataListeners(companyId) {
    
    detachAllCompanyListeners();
    ...
    unsubscribeReturns = onSnapshot(
        collection(db, "companies", companyId, "returns"), 
        (snapshot) => { 
            window.returnsList = []; 
            snapshot.forEach(doc => { 
    ...
    unsubscribeArchive = onSnapshot(
        collection(db, "companies", companyId, "archive"), 
        (snapshot) => { 
            window.shortageArchive = []; 
            snapshot.forEach(doc => { 
    ...
    unsubscribeFactoryOrders = onSnapshot(
        collection(db, "companies", companyId, "factory_orders"), 
        (snapshot) => { 
            window.factoryOrders = []; 
            snapshot.forEach(doc => { 
...
/**
 * 💡 [الحفاظ على الجلسة وعرض هوية الشركة]
 */
onAuthStateChanged(auth, async (user) => {
    if (user) {
        const savedUser = localStorage.getItem('erp_username');
        const savedCompany = localStorage.getItem('erp_company_id');
        
        if (savedUser && savedCompany) {
            
            const userDocId = savedCompany.toLowerCase() + "_" + savedUser;
            
            const qSnap = await getDocs(
                query(
                    collection(db, "users"), 
                    where("uniqueId", "==", userDocId)
                )
            );
            ...
            if (!qSnap.empty) {
                ...
                window.currentCompanyId = data.companyId;
                
                document.getElementById('display-manager').innerText = `🏢 شركة: ${data.companyId} | 👤 ${data.username} (${data.shift})`;
                ...
                initCompanyDataListeners(data.companyId);
            }
        }
    }
});
...
/**
 * 💡 [الموردين والمناديب الفعليين للمخزن للواتساب]
 */
window.suppliersDB = [
    { 
        keywords: [
        ...
        factory: "مورد خامات الحلويات والمخابز", 
        whatsapp: "201000000000", 
        contact: "مندوب منطقة الدقهلية" 
    },
    { 
        keywords: [
            "رز", 
        ...
        factory: "موزع السلع التموينية", 
        whatsapp: "201100000000", 
        contact: "وكيل المضارب" 
    },
    { 
        keywords: [
            "زيت", 
        ...
        factory: "مندوب شركة أرما/كريستال", 
        whatsapp: "201200000000", 
        contact: "مندوب قطاع التجزئة" 
    },
    { 
        keywords: [
            "كرتون", 
...
window.updateProductPrice = async (docId) => {
    ...
    try {
        await updateDoc(
            doc(db, "companies", window.currentCompanyId, "products", docId), 
            { 
                price: newPriceValue 
            }
        );
...
/**
 * 💡 [التحكم في القائمة الجانبية بذكاء]
 */
window.toggleSidebar = () => {
    document.getElementById('app-sidebar').classList.toggle('active');
};

// مراقبة الضغطات لغلق القائمة عند الضغط في مكان فارغ بدون عناصر وهمية
document.addEventListener('click', (event) => {
    const sidebar = document.getElementById('app-sidebar');
    const menuBtn = document.querySelector('.mobile-menu-btn');
    
    // التأكد من وجود القائمة وأنها مفتوحة بالفعل
    if (sidebar && sidebar.classList.contains('active')) {
        // إذا كان مكان الضغطة لا يقع داخل القائمة، ولا يقع على زر الفتح
...
/**
 * 💡 [تأسيس شركة جديدة من الخارج - Super Admin]
 */
window.registerCompany = async () => {
    const companyId = document.getElementById('reg-company-id').value.trim().toLowerCase();
    const username = document.getElementById('reg-admin-user').value.trim();
    const shift = document.getElementById('reg-admin-shift').value;
    const password = document.getElementById('reg-admin-pass').value.trim();
    
    if (!companyId || !username || !shift || !password) {
        return window.showToast("يرجى إكمال جميع بيانات الشركة!", "error");
    }
    
    if (password.length < 6) {
        return window.showToast("كلمة المرور يجب أن تكون 6 أحرف على الأقل!", "error");
    }

    const secureEmail = window.encodeUserForEmail(companyId, username);
    const uniqueDocId = companyId + "_" + username;

    try {
        await createUserWithEmailAndPassword(auth, secureEmail, password);
        
        await setDoc(
            doc(db, "users", uniqueDocId), 
            { 
        ...
        window.showToast("تم تأسيس الشركة وحفظ المدير بنجاح!", "success");
        window.closeModal('register-company-modal');
        
    } catch (e) { 
        window.showToast("خطأ أثناء التأسيس: تأكد أن الشركة غير مسجلة مسبقاً", "error"); 
    }
};

/**
 * 💡 [إضافة موظف من داخل النظام - Manager]
 */
window.openAddEmployeeModal = () => {
    document.getElementById('display-company-code-modal').innerText = `إضافة فرع أو موظف لشركة: ( ${window.currentCompanyId} )`;
    window.openModal('add-employee-modal');
};
...
window.registerEmployee = async () => {
    ...
    if (!username || !password || !shift) {
        return window.showToast("يرجى إكمال بيانات الموظف!", "error");
    }
    
    if (password.length < 6) {
        return window.showToast("كلمة المرور قصيرة جداً!", "error");
    }
    ...
    try {
        ...
        window.showToast("تم إضافة الموظف بنجاح لشركتك!", "success");
        window.closeModal('add-employee-modal');
        
        document.getElementById('emp-user').value = '';
        document.getElementById('emp-pass').value = '';
        
    } catch (e) { 
        window.showToast("خطأ: تأكد أن اسم الموظف غير مستخدم في شركتك", "error"); 
    }
};
 ...
 */
window.login = async () => {
    const companyId = document.getElementById('login-company-id').value.trim().toLowerCase();
    const username = document.getElementById('login-username').value.trim();
    const password = document.getElementById('login-password').value.trim();
    
    if (!companyId || !username || !password) {
        return window.showToast("يرجى إدخال كود الشركة وبيانات الدخول", "error");
    }
    ...
    try {
        ...
        if (!qSnap.empty) {
            ...
            localStorage.setItem('erp_company_id', data.companyId);
            ...
            window.showToast("بيانات المستخدم غير موجودة", "error"); 
        }
    } catch (e) { 
        window.showToast("اسم المستخدم أو كلمة المرور أو كود الشركة غير صحيح", "error"); 
    }
};
...
window.submitFactoryOrder = async () => {
    ...
    let targetWhatsapp = supplier ? supplier.whatsapp : "";
    let fName = supplier ? supplier.factory : "مورد عام";
    
    try {
        await addDoc(
            collection(db, "companies", window.currentCompanyId, "factory_orders"), 
            { 
                product: name, 
                qty: qty, 
                unit: unit, 
                ...
                userPhone: userPhone, 
                factory: fName, 
                contact: supplier ? supplier.contact : "غير مسجل", 
                status: "تم إرسال الطلب عبر الواتساب ⏳", 
                timestamp: serverTimestamp(), 
                orderedBy: window.currentUser 
            }
        );
        
        window.showToast("تم تسجيل الطلب بالنظام.. جاري فتح الواتساب", "success"); 
        window.closeModal('factory-order-modal');

        const messageBody = encodeURIComponent(`السلام عليكم،\nمطلوب توريد طلبية جديدة من فضلك:\n\n📦 *المنتج:* ${name}\n⚖️ *الكمية:* ${qty} ${unit}\n\n🏢 *اسم المخزن:* ${window.currentUser} (فرع: ${window.currentCompanyId})\n📍 *عنوان الاستلام:* ${address}\n📞 *رقم التواصل:* ${userPhone}\n\nبرجاء تأكيد ميعاد التوريد وإجمالي الفاتورة. شكراً.`);
        
        if (targetWhatsapp) {
            window.open(`https://wa.me/${targetWhatsapp}?text=${messageBody}`, '_blank');
        } else {
...
window.cancelFactoryOrder = async (orderId) => {
    try {
        await deleteDoc(
            doc(db, "companies", window.currentCompanyId, "factory_orders", orderId)
        );
...
window.addIncoming = async () => {
    ...
    try {
        await setDoc(
            doc(db, "companies", window.currentCompanyId, "products", code), 
            { 
                code: code, 
                name: name, 
                category: category, 
...
window.submitDispense = async () => {
    ...
    try {
        await updateDoc(
            doc(db, "companies", window.currentCompanyId, "products", product.id), 
            { 
                qty: increment(-qty) 
            }
        );
        ...
        if (product.qty - qty <= 0) {
            await setDoc(
                doc(db, "companies", window.currentCompanyId, "archive", code), 
                { 
                    ...product, 
                    qty: 0, 
                    status: "نفذت الكمية 🔴" 
...
window.addReturn = async () => {
    ...
    try {
        ...
        await addDoc(
            collection(db, "companies", window.currentCompanyId, "returns"), 
            { 
                code: code, 
                name: pName, 
                customer: customer, 
...
async function addMovementLog(code, name, type, qty, price, receiverName, receiverPhone) {
    ...
    await addDoc(
        collection(db, "companies", window.currentCompanyId, "movements"), 
        { 
            date: todayDateStr, 
    ...
    let reportHTML = `
        ...
        <head>
            ...
            <style>
                ...
                /* إخفاء الزر أثناء الطباعة الورقية، وإخفاء حدود المربع ليظهر كأنه كلام مطبوع */
                @media print {
                    .print-btn { 
        ...
        <body>
            ...
            <div class="footer">
                <div>
                    <p style="margin-bottom: 15px;">إجمالي الحركات اليوم: ${window.dailyMovements.length} حركة</p>
                    <p>توقيع أمين المخزن: <strong>${window.currentUser}</strong></p>
                </div>
                
                <div style="text-align: right;">
                    <p style="margin-bottom: 15px;">
                        <label>اسم المستلم: </label>
                        <input type="text" class="custom-input" placeholder="اكتب الاسم هنا...">
                    </p>
                    <p>
                        <label>رقم الفون: </label>
                        <input type="text" class="custom-input" placeholder="اكتب الرقم هنا...">
                    </p>
                </div>
            </div>
