...
/**
 * 💡 [خوارزمية / تشفير الأسماء للإيميل]:
 * - الوظيفة: تحويل الاسم (عربي/إنجليزي/مسافات) إلى كود آمن يقبله فايربيز كإيميل.
 * - الآلية: تحويل كل حرف في الاسم لرمز سداسي عشري (Hex) لضمان عدم وجود مسافات أو حروف غير مقبولة.
 */
 ...
 * - التعديل: دعم كامل للحروف العربية والمسافات وشرط باسورد لا يقل عن 6 أرقام.
 */
window.registerUser = async (role) => {
    ...
    // تشفير الاسم عشان الفايربيز يقبله بدون مشاكل
    const safeEmailPrefix = window.encodeUsernameForEmail(username);

    try {
        await createUserWithEmailAndPassword(auth, `${safeEmailPrefix}@mastech.erp`, password);
        
        // حفظ الاسم الأصلي في الداتابيز عادي جداً
        ...
        window.showToast("حدث خطأ أثناء التسجيل. تأكد أن الاسم غير مستخدم مسبقاً", "error"); 
    }
};
...
window.login = async () => {
    ...
    // تشفير الاسم زي ما عملنا في التسجيل عشان يعرف يقارنه في الفايربيز
    const safeEmailPrefix = window.encodeUsernameForEmail(u);

    try {
        await signInWithEmailAndPassword(auth, `${safeEmailPrefix}@mastech.erp`, p);
        
        // البحث عن بيانات المستخدم بالاسم العادي في الداتابيز
